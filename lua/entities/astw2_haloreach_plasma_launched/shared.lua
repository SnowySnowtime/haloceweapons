...
function ENT:PhysicsCollide(data, physobj)
    if SERVER then
        self:EmitSound(Sound("halo/halo_reach/weapons/pl_proj_attached" .. math.random(1,3) .. ".ogg"))
...
function ENT:Think()
    ...
    if SERVER then
        ...
        if CurTime() >= self.at then
            local targets = ents.FindInSphere(self:GetPos(), 512)
            for _, k in pairs(targets) do
                if k:IsPlayer() or k:IsNPC() and (CurTime() >= self.bt) then
                    if self:Visible( k ) and k:Health() > 0 and (CurTime() >= self.bt) then
                        self:Detonate()
                    end
                elseif (k:IsValid() and scripted_ents.IsBasedOn( k:GetClass(), "base_nextbot" ) and (CurTime() >= self.bt)) then
                    self:Detonate()
                end
            end
        end

        if CurTime() >= self.bt then
            self:Detonate()
        end
    end
...
function ENT:Detonate()
    if SERVER then
        ...
        if self:WaterLevel() >= 1 then
            ...
            util.Effect( "astw2_haloreach_explosion_stickylauncher", effectdata)
			sound.Play( "halo/halo_reach/weapons/pl_proj_expl" .. math.random(1,3) .. ".ogg",  self:GetPos(), 100, 100 )
