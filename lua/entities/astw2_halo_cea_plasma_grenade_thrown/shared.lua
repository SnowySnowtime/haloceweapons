...
function ENT:Initialize()
    if SERVER then
        ...
        self.bt = CurTime() + 2.4
	self.Armed = false
	 self.motorsound = CreateSound( self, "halo/combat_evolved_anniversary/plas_grenlp" .. math.random(1,3) .. ".ogg")
...
function ENT:PhysicsCollide(data, physobj)
	...
	self:EmitSound(Sound("halo/combat_evolved_anniversary/detonation_start" .. math.random(1,3) .. ".ogg."))
...
function ENT:Think()
    ...
    if CLIENT then
        ...
        local smoke = emitter:Add("effects/halo_reach/plasma_bolt", self:GetPos())
...
function ENT:Detonate()
    if SERVER then
        ...
        if self:WaterLevel() >= 1 then
			...
			sound.Play( "halo/combat_evolved_anniversary/plasmagx" .. math.random(1,5) .. ".ogg",  self:GetPos(), 100, 100 )
