...
self.flightvector = self.Entity:GetForward() * ((100*10.5)/15)
self.timeleft = CurTime() + 900
self.Owner = self:GetOwner()
self.Entity:SetModel( "models/snowysnowtime/projectiles/rocket_projectile.mdl" )
self.Entity:PhysicsInit( SOLID_VPHYSICS )      -- Make us work with physics,  	
self.Entity:SetMoveType( MOVETYPE_NONE )   --after all, gmod is a physics  	
self.Entity:SetSolid( SOLID_VPHYSICS )        -- CHEESECAKE!    >:3    
self:EmitSound("rl_rockettrail", 60)
...
Glow:SetKeyValue("rendercolor","255 195 0")
	...
	local tr = util.TraceLine( trace )
		...
		if tr.Hit then
				local effectdata = EffectData()
					...
					util.Effect( "snow_hce_rl_explode", effectdata )
					util.BlastDamage(self.Entity, self:OwnerGet(), tr.HitPos, 250, 770)
					util.Decal("crater", tr.HitPos + tr.HitNormal, tr.HitPos - tr.HitNormal)
					self:StopSound("rl_rockettrail")
					self.Entity:SetNWBool("smoke", false)
					
			self:Explosion()
			self.Entity:EmitSound( "hce/grenade_expl_"..math.random(1,2)..".wav", SNDLVL_512dB, 800, 4 )
			self:StopSound("rl_rockettrail")
			self.Entity:Remove()	
		end
