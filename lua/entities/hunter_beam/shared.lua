...
self.flightvector = self.Entity:GetForward() * ((10*10)/10)
	...
	local tr = util.TraceLine( trace )
		...
		if tr.Hit then
				local effectdata = EffectData()
					...
					util.Effect( "astw2_haloreach_explosion_hunterbeam", effectdata )
					util.BlastDamage(self.Entity, self:OwnerGet(), tr.HitPos, 75, 250)
	...
	pos = self:GetPos()
		for i=1, (1) do
			...
			if (particle) then
				...
				particle:SetStartSize( math.Rand( 25, 62 ) )
				particle:SetEndSize( math.Rand( 12, 12 ) )
		...
		for i=1, (2) do
			...
			if (particle) then
				...
				particle:SetStartAlpha( math.Rand( 7, 10 ) )
				particle:SetEndAlpha( 255 )
				particle:SetStartSize( math.Rand( 25, 25 ) )
				particle:SetEndSize( math.Rand( 18, 18 ) )
				particle:SetRoll( math.Rand(0, 360) )
				particle:SetRollDelta( math.Rand(-1, 1) )
