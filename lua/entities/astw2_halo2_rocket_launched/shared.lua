...
function ENT:Detonate()
    if SERVER then
        ...
        if self:WaterLevel() >= 1 then
            ...
            util.Effect( "astw2_halo2_explosion_rocket", effectdata)
    ...
    if CLIENT then
        ...
        if self.Armed then
            cam.Start3D() -- Start the 3D function so we can draw onto the screen.
                render.SetMaterial( Material("effects/halo_ce/flare_generic") ) -- Tell render what material we want, in this case the flash from the gravgun
                render.DrawSprite( self:GetPos(), math.random(35, 50), math.random(85, 100), Color(255, 255, 150) ) -- Draw the sprite in the middle of the map, at 16x16 in it's original colour with full alpha.
            cam.End3D()
        end
    end
end