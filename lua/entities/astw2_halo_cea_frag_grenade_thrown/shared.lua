...
function ENT:Initialize()
    if SERVER then
		...
		self:EmitSound(Sound("halo/combat_evolved_anniversary/throw5_" .. math.random(1,3) .. ".ogg"))
    end
end
...
function ENT:PhysicsCollide(data, physobj)
    if SERVER then
        if data.Speed > 75 then
            self:EmitSound(Sound("halo/combat_evolved_anniversary/grenbouncemetal_" .. math.random(1,5) .. ".ogg"))
        end
...
function ENT:Detonate()
	...
	sound.Play( "halo/combat_evolved_anniversary/frag" .. math.random(1,4) .. ".ogg",  self:GetPos(), 100, 100 )
